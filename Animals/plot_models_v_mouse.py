...
def plot_by_block_position(bpos, subset='condition', **kwargs):
    ...
    ax1.set(xlim=(-10,20), ylim=(0,1), xlabel='Block Position', ylabel='P(optimal)')
...
def calc_confusion_matrix(df_mouse, col, df_model=None):
    ...
    if df_model is None:
        ...
        # print(np.all(df_mouse.history.tolist() == df_model.history.tolist()) )
        assert( np.all(df_mouse.history.tolist() == df_model.history.tolist()) )
    
    N = df_mouse.n.tolist() # same counts for model
    a = df_mouse[col].tolist()
    b = df_model[col].tolist()
